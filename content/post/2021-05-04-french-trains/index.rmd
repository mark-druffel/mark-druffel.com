...
author: Mark Druffel
date: '2021-05-04'
slug: french-train-delays
categories: ["tidy-tuesday"]
tags: ["tidyverse", "tidygraph", "sf", 'sfnetworks', 'leaflet']
output:
  blogdown::html_page:
    toc: true
    toc_depth: 3
...
```{r setup, include = F, message = F, warning = F, echo = T, cache = T}
...
```{r network_layouts_gif, fig.show='animate', ffmpeg.format='gif', dev='jpeg'}
...
```{r, network_clusters_gif, fig.show='animate', ffmpeg.format='gif', dev='jpeg'}
group_edge_betweenness_map_n_dfc <- function(tbl_graph, i){
...
group_edge_betweenness_map_n_plots <- function(tbl_graph){
  grp_cols <- unique(tbl_graph  %N>% pull(grp_cols)) |> 
    unlist()
  plots <- map(grp_cols, function(col){
    tbl_graph |>
      ggraph(layout = "stress") +
      geom_edge_link0(width = 0.15) +
      geom_node_point(aes(size = arrivals + departures, fill = eval_tidy(data_sym(col))), show.legend = F) +
      geom_mark_hull(
        aes(x, y, group = eval_tidy(data_sym(col)), fill =  eval_tidy(data_sym(col)), label = eval_tidy(data_sym(col))),
        concavity = 4,
        expand = unit(2, "mm"),
        alpha = 0.25
      ) +
      scale_fill_brewer(palette = "Set1") +
      labs(title = paste0("Groups: ", stringr::str_remove(col, "grps_")), x = NULL, y = NULL, fill = "Group") +
      theme_blog_network()
  })
  return(plots)
}
plots <- network_df |> 
  group_edge_betweenness_map_n_dfc(i = 2:8) |> 
  group_edge_betweenness_map_n_plots()
walk(plots, print)
```
...
```{r arc_map_gif, fig.width = 10, fig.show='animate', ffmpeg.format='gif', dev='jpeg'}
