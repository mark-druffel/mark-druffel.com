...
knitr::opts_chunk$set(echo = TRUE, include = T, result = T, message = F, error = F, fig.width = 9)
library(knitr)
library(markUtils)
library(tidyverse)
library(janitor)
...
I went down a few rabbit holes that I did not write-up. I built a sunburst of the year, brewery, category, and beers. It was too difficult to read even with factor lumping because the dispersion in the data. I tried to use a crosstalk SharedData object to link the sunburst to another higher view, but it turns out [sunburstR](https://github.com/timelyportfolio/sunburstR) does not support crosstalk at the moment (side note, if you have not used sunburstR it makes beautiful d3.js sunbursts with very little effort). I also tried to figure out how to clean up the character issues such as ä, but I couldn't find any simple tools to use for that specific issue. 
